...
const Bubble = (props) => {
  ...
  if (item.type === MessageType.SYSTEM_EVENT) {
    ...
    const { type: typeSystemEvent } = payloadSystemEvent
      ? payloadSystemEvent
      : {};
    if (typeSystemEvent !== MessageType.SYSTEM_EVENT_SEQUENCE_QUEUE)
      return null;
  }
  ...
  const showAvatar =
    item.type !== MessageType.SYSTEM_EVENT &&
    !isMyMessage &&
    item.email !== beforeItem?.email;
  ...
  const getAvatar = () => {
    ...
    if (state.avatar) {
      return <Image style={styles.avatar} source={{ uri: state.avatar }} />;
    }
    ...
    <View style={styles.container}>
      ...
      {item.type !== MessageType.SYSTEM_EVENT && (
        ...
        >
          ...
          <View
            style={{
              paddingLeft: !showAvatar
                ? styles.avatar.width + styles.avatar.marginRight
                : 0,
                    ...
                    style={[
                      ...
                      },
                      {
                        backgroundColor: isMyMessage
                          ? CONFIG.backgroundBubbleRight
                          : CONFIG.backgroundBubbleLeft,
                      },
                    ]}
                    onLongPress={() => {
