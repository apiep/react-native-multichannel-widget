    ...
    <View>
      ...
      >
        ...
        <View style={{ padding: 5 }}>
          {replied_comment_type !== MessageType.UNKNOWN && (
            <Text style={{ fontWeight: 'bold' }}>
              {replied_comment_sender_username}
            </Text>
          )}
          {replayItem(payload)}
        </View>
...
const replayItem = (payload) => {
  ...
  if (
    replied_comment_type === MessageType.TEXT ||
    replied_comment_type === MessageType.REPLY
  ) {
    return (
  ...
  if (replied_comment_type === MessageType.ATTACHMENT) {
    ...
    if (isImageFile(url)) {
      ...
      return <MessageAttachment hideDownloadButton={true} item={message} />;
    }
  }
};
