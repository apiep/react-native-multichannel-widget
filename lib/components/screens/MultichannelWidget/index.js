...
import { getMoment as moment, isEmpty, qiscusLogger, qiscusToast } from '../../../utils';
...
function MultichannelWidget(props) {
	const {
		onSuccessGetRoom,
		onTyping,
  ...
  const qiscus = useQiscus()
	...
	const { room, deleteMessage, sendMessage: _sendMessage, loadMoreMessages } = useCurrentChatRoom();
	...
	const isLogin = useMemo(() => roomId !== 0 && roomId !== null && currentUser != null, [roomId, currentUser]);
	const loading = useMemo(() => currentUser == null || room == null, [currentUser, room]);
	...
	const onOpenActionMessage = (item) => {
		if (item.type !== MessageType.TEXT && item.type !== MessageType.REPLY && item.type !== MessageType.ATTACHMENT) {
			return;
		}
	...
	const _onPressSendAttachment = () => {
		...
		onPressSendAttachment()
			.then((opts) => {
				...
				setAttachment({ type: MessageType.IMAGE, value: opts.uri, payload: null })
				const source = {
					uri: opts.uri,
					name: opts.name,
					type: opts.type,
				...
				qiscus.upload(source, (error, progress, fileUrl) => {
					...
					if (error != null) {
						...
						console.log('error uploading image', JSON.stringify(error, null, 2))
						setUploadUrl('');
						return;
					}
					if (fileUrl != null) {
						...
						if (source.uri === uploadUrl) setAttachment({ type: MessageType.IMAGE, value: opts.uri, payload });
						setUploadUrl('');
					}
	...
	const resetPayloadAndMessage = () => {
		if (state.progressUploading === 100 || state.progressUploading === -1 || state.progressUploading === '') {
			setAttachment({type: '', value: '', payload: ''})
		}
		setProgressUploading('')
    setReplayMessage({})
	...
	const sendMessagePostBack = useCallback((postback_text, payload) => {
