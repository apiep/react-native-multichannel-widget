 ...
 * @returns {{
 * 	state: AppState,
 *  setup(appId: string, options?: Options): Promise<void>,
 *  initiateChat(userId: string, displayName: string, options: Options): Promise<void>,
 * }}
 */
    ...
    await qiscus.init({
      ...
      options: {
        newMessagesCallback: (messages) =>
          messages.forEach((m) => events.emit(event.messageReceived, m)),
        commentReadCallback: (message) =>
          events.emit(event.messageRead, message),
        commentDeliveredCallback: (message) =>
          events.emit(event.messageDelivered, message),
        commentDeletedCallback: (message) =>
          events.emit(event.messageDeleted, message),
